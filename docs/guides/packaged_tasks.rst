...
The following config loads the tasks generated by running the ``get_tasks`` function from the ``mypkg`` package. The package must be accessible to the default :ref:`executor<Configure the executor for a task>`, i.e. it must be within the current project or its dependencies.
...
Normally Poe the Poet detects the most appropriate :ref:`executor<Configure the executor for a task>` (i.e. project virtualenv) to use to find and execute the referenced package. However you can customize this behavior by specify an executor for the script like so:
