def test_expr_with_args(run_poe):
    ...
    result = run_poe("expr_with_args", "999", project="expr")
    assert result.capture == (
        """Poe => "power level is " + (f"only {power_level}" """
        """if int(${power_level}) <= 9000 else 'over nine thousand!\')\n"""
    )
    ...
    result = run_poe("expr_with_args", "9001", project="expr")
    assert result.capture == (
        """Poe => "power level is " + (f"only {power_level}" """
        """if int(${power_level}) <= 9000 else 'over nine thousand!\')\n"""
    )
...
def test_expr_with_argv(run_poe):
    result = run_poe("expr_with_argv", "999", project="expr")
    assert result.capture == (
    ...
    result = run_poe("expr_with_argv", "9001", project="expr")
    assert result.capture == (
        """Poe => "power level is " + (f"only {sys.argv[1]}" """
        """if int(sys.argv[1]) <= 9000 else 'over nine thousand!\')\n"""
    )
...
def test_expr_with_env_vars(run_poe):
    result = run_poe(
        "expr_with_env_vars",
        "999",
        project="expr",
...
def test_expr_with_imports(run_poe):
    result = run_poe("expr_with_imports", project="expr")
    assert result.capture == "Poe => bool(re.match(r'^\\S+@\\S+\\.\\S+$', ${EMAIL}))\n"
    assert result.stdout == "True\n"
    assert result.stderr == ""

    result = run_poe("expr_with_imports", project="expr", env={"EMAIL": "lol"})
    assert result.capture == "Poe => bool(re.match(r'^\\S+@\\S+\\.\\S+$', ${EMAIL}))\n"
...
def test_expr_with_uses(run_poe):
    result = run_poe("expr_with_uses", project="expr")
    assert result.capture == (
        "Poe <= 0\n"
...
def test_christmas_tree_expr(run_poe):
    result = run_poe("christmas_tree_expr", project="expr")
...
def test_expr_multi_value_empty_flag(run_poe):
    ...
    result = run_poe("multi_value_expr", "--nums", "--words", project="expr")
    assert result.stdout == "{'nums': [], 'words': []}\n"


def test_expr_private_arg_accessible(run_poe):
    """Private args are accessible as typed values in expr tasks"""
    result = run_poe("private_arg_expr", project="expr")
    assert result.stdout == "{'_flag': True, '_FLAG': True}\n"


def test_expr_private_arg_negated(run_poe):
    """Inferred option names strip leading underscores for expr arg declarations too"""
    result = run_poe("private_arg_expr", "--flag", project="expr")
    assert result.stdout == "{'_flag': False, '_FLAG': True}\n"


def test_expr_env_access_uses_typed_and_templated_channels(run_poe):
    result = run_poe("bool_env_access_expr", project="expr")
